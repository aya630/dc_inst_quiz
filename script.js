...
function showQuestion() {
 ...
 options.forEach((option) => {
    ...
    button.dataset.value = option; // 正誤判定のために裏側で文字を覚えさせておきますわ

    const imageNumber = option.substring(0, 2);

    // 【修正】文字（<span>）を削除し、画像だけをボタンの中に入れますわ！
    button.innerHTML = `
        <img src="images/${imageNumber}.jpg" alt="${option}" class="option-image">
    `;

    // Easyモードの処理ですわ（ここは元のままです）
    if (isEasyMode && option !== question.correctAnswer && answeredCorrectly.includes(option)) {
        button.style.visibility = 'hidden'; 
        button.disabled = true;             
    } else {
        // 【修正】event.target ではなく、変数 button を直接 selectAnswer に渡すようにしますわ
        button.addEventListener('click', () =>
          selectAnswer(option === question.correctAnswer, button)
        );
    }
...
const allButtons = optionsContainer.querySelectorAll('button');
  allButtons.forEach((btn) => {
    ...
    // 【変更】 btn.textContent を btn.dataset.value に書き換えますわ！
...
function saveScoreAndShowRankings() {
  ...
  const accuracy =
    currentQuestions.length > 0
      ...
      : 0;
  const now = new Date();
  const timestamp = now.getTime(); // ★ 識別のためのタイムスタンプを追加
  ...
  const newRecord = {
    ...
    timestamp: timestamp, // ★ 保存するデータにタイムスタンプを含める
  };
...
function renderRankingTable(rankings) {
  const latestTimestamp = sessionStorage.getItem('latestScoreTimestamp'); // ★ 最新スコアの情報を取得
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (localStorage.getItem('darkMode') === 'enabled') {
      // もし 'enabled' なら、body要素に 'dark-mode' クラスを追加する
